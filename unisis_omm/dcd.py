...
class DcdFile :
    ...
    def read_header(self):
        if not self._file :
            raise Exception('_file is None in DcdFile read_header.')
        ...
        #if bdata[0] != 'CORD' :
        #    raise Exception(f'{self._filename} is not a coordinate file (DcdFile, read_header).')
    ...
    def write_header(self):
        if not self._header :
            raise Exception('DcdFile write_header: _header is None')
    ...
    def read_onestep_omm(self):
        ...
        for i in range(self._header.nmp_real) :
            xyz = [x[i] * unit.angstrom, y[i] * unit.angstrom, z[i] * unit.angstrom]
            coord_matrix.append(xyz)
